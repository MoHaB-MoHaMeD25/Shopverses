fetch('https://mohab-mohamed25.github.io/Shopverses/products.json')
...
.then(data => {
    ...
    data .forEach(product => {
        if(product.category == "electronics"){
            ...
            const persent_disc_div = product.old_price ? `<span class="sale_present">%${Math.floor((product.old_price - product.price ) / product.old_price * 100)}</span>` : "" ;
...
setTimeout(() => {
    [hotSwiper, elecSwiper, applSwiper, mobileSwiper].forEach(sw => {
        sw.update();
        sw.updateSlides();
        sw.updateProgress();
        sw.slideToLoop(0, 0);
